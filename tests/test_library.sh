...
LIBRARY_TESTS="error path support system_string";
...
run_test()
{
	...
	local TEST_EXECUTABLE="./cpath_test_${TEST_NAME}";
...
run_test_with_input()
{
	...
	run_test_on_input_directory "libcpath" "${TEST_DESCRIPTION}" "default" "${OPTION_SETS}" "${TEST_EXECUTABLE}" "input" "${INPUT_GLOB}";
	local RESULT=$?;
